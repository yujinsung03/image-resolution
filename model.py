...
class Attention(nn.Module):
    def __init__(self, channels, num_heads, bias):
        ...
        self.qkv_conv = nn.Conv2d(channels * 3, channels * 3, kernel_size=3, padding=1, groups=channels * 3, bias=bias)
...
class Restormer(nn.Module):
    def __init__(self,
        ...
        out_channels=3,
        dim=48,
        num_blocks=[4, 6, 6, 8, 4],  # Updated for level 5
        num_refinement_blocks=4,
        heads=[1, 2, 4, 8, 16],  # Updated for level 5
        ...
        self.down4_5 = Downsample(int(dim * 2 ** 3))  # New downsampling layer
        self.latent = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 4), num_heads=heads[4], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type)
            for i in range(num_blocks[4])
        ])

        self.up5_4 = Upsample(int(dim * 2 ** 4))  # New upsampling layer
    ...
    def forward(self, inp_img):
        ...
        inp_enc_level5 = self.down4_5(out_enc_level4)  # New downsampling step
        latent = self.latent(inp_enc_level5)

        inp_dec_level4 = self.up5_4(latent)  # New upsampling step
